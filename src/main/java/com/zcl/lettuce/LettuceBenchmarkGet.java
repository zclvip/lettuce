...
public class LettuceBenchmarkGet {
...
//        orderInfo = createOrderInfo();
    ...
    public void getOrder(){
        ...
        byte[] data = sync.get((key + id).getBytes());
        if (data != null){
            OrderInfo orderInfo = kryo.Deserialize(data);
        }else {
            return;
        }
    }
    ...
    public static void main(String[] args) throws RunnerException {
        Options options = new OptionsBuilder()
                .include(LettuceBenchmarkGet.class.getSimpleName())
                .forks(1)
                .build();
